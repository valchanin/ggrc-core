...
class WithEvidence(object):
  ...
  _fulltext_attrs = [
      ...
      MultipleSubpropertyFullTextAttr('evidences_url', 'evidences_url',
                                      ['link']),
      MultipleSubpropertyFullTextAttr('evidences_reference_url', 'evidences_reference_url',
                                      ['link']),
  ]
  ...
  @declared_attr
  def evidences(cls):
    """Return evidences releated for that instance."""
    evidence_id = case(
        [
            (
    ...
    return db.relationship(
        ...
        primaryjoin=lambda: and_(with_evidence_obj_id, cls.id == with_evidence_obj_id),
        ...
        secondaryjoin=lambda: and_(evidence_id, Evidence.id == evidence_id,
                                   with_evidence_obj_type == cls.__name__),
        viewonly=True,
    )
  ...
  @property
  def evidences_reference_url(self):
    return self.get_evidences_by_kind(Evidence.REFERENCE_URL)
  ...
  def log_json(self):
    ...
    out_json['evidences_file'] = self._log_evidences(
      self.evidences_file)
    out_json['evidences_reference_url'] = self._log_evidences(
      self.evidences_reference_url)
    return out_json
