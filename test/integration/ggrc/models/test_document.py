...
class TestDocument(TestCase):
  ...
  def test_documentable_postfix_two_controls(self):
    ...
    with factories.single_commit():
      ...
      assessment = factories.AssessmentFactory(audit=audit)
      factories.RelationshipFactory(source=assessment, destination=snapshots[0])
      factories.RelationshipFactory(source=assessment, destination=snapshots[1])
    ...
    expected = '_ggrc_assessment-{}_control-{}_control-{}'.format(assessment.id,
                                                                  control1.id,
                                                                  control2.id)
    result = document._build_file_name_postfix(assessment)
    self.assertEqual(expected, result)

  @mock.patch('ggrc.gdrive.file_actions.copy_file', dummy_gdrive_responce)
  def test_documentable(self):
    """Test update document title."""

    with factories.single_commit():
      audit = factories.AuditFactory(folder='1H0VIe5iZYPB8t9ACPhSpuxuZvrPLeD-S')
      assessment = factories.AssessmentFactory(audit=audit)
